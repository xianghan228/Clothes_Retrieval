...
class DataProvider:
  ...
  def read_imgs(self):
    ...
    f = open(self.path,'r')
    for line in f.readlines():
      img_dict[line.strip().split()[0]] = line.strip().split()[1]
    return img_dict
  ...
  def get_batch_vec(self,p1=56,p2=84,p3=140,p4=168):
    ...
    img_nps_org = np.zeros((self.batchsize,3,self.img_size,self.img_size)).astype(np.float32)
    ...
    while counter<self.batchsize:
      ...
      img_name = os.path.basename(self.img_list[self.batch_index])[:-4]
      img = img.resize((self.img_size,self.img_size))
      img_np = np.asarray(img)
      img_split=[]
      img_split.append(Image.fromarray(img_np[:p2,:,:]))
      ...
      img_np = img_np-np.load('../mean_227.npy')[0]

      img_nps_1[counter,:,:,:]   = img_np_i[0]
      img_nps_2[counter,:,:,:]   = img_np_i[1]
      img_nps_3[counter,:,:,:]   = img_np_i[2]
      img_nps_org[counter,:,:,:] = img_np
      labels[counter]          = self.img_dict[self.img_list[self.batch_index]]
      ...
      if self.batch_index < self.batch_len-1:
        ...
        random.shuffle(self.img_list)
      counter += 1
    return img_nps_1, img_nps_2, img_nps_3, labels, img_nps_org, self.epoch_counter, img_names
...
class DataProviderLayer(caffe.Layer):
  ...
  def forward(self,bottom,top):
    img_nps_1, img_nps_2, img_nps_3, labels, img_nps_org, _ , _= self.dp.get_batch_vec()
    top[0].data[...] = img_nps_1
    top[1].data[...] = img_nps_2
    top[2].data[...] = img_nps_3
    top[3].data[...] = labels
    top[4].data[...] = img_nps_org
