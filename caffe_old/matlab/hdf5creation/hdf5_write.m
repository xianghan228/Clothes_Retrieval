...
data_disk=rand(1,4096,num_total_samples); 
...
for batchno=1:num_total_samples/chunksz
  ...
  % store to hdf5
  startloc=struct('dat',[1,1,totalct+1], 'lab', [1,totalct+1]);
...
data_rd=h5read(filename, '/data', [1 1 800], [1,4096, 100]);
label_rd=h5read(filename, '/label', [1 800], [1, 100]);
fprintf('Testing ...\n');
try 
  assert(isequal(data_rd, single(data_disk(:,:,800:899))), 'Data do not match');
  assert(isequal(label_rd, single(label_disk(:,800:899))), 'Labels do not match');
