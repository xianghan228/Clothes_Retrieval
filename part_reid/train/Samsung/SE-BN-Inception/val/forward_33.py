...
if __name__ == '__main__':
  for i in [70000,75000,80000,85000,90000,95000,100000]:
    feat_extract(img_list_path='./evaLib33.txt',model='../train/snapshot_sambox/model_iter_%d.caffemodel'%i,iter=i,flag='_sambox')
    feat_extract(img_list_path='./evalib33.txt',model='../train/snapshot_sambox/model_iter_%d.caffemodel'%i,iter=i,flag='_sambox')
    feat_extract(img_list_path='./query33.txt',model='../train/snapshot_sambox/model_iter_%d.caffemodel'%i,iter=i,flag='_sambox')
    while(not os.path.exists('../train/snapshot/model_iter_'+str(i+5000)+'.caffemodel')):
      time.sleep(10)
