...
class DataProvider:
    def __init__(self, params):
        ...
        with open('/data1/qtang/samsung/part_reid/train/Samsung/alidfcd_ivabox_sambox_partnet_baseline/hard_neg_top40_new_baseline.pkl','r') as f:
          self.q_neg = pickle.load(f)
        with open('/data1/qtang/samsung/part_reid/train/Samsung/alidfcd_ivabox_sambox_partnet_baseline/label_query.pkl','r') as f:
          self.label_query = pickle.load(f)
            ...
            if id in self.q_neg:
              for file_name in self.q_neg[id]:
                if self.count < self.batch_size:
                  ...
                  self.count=0
                  return batch_list
            if id in self.label_query:
              if self.count < self.batch_size:
                batch_list.append((self.label_query[id],id))
                self.count += 1
              else:
                self.index=idx%len(self.all_id)
                self.count=0
