...
def get_trian_array_and_label(path_npz='/home/qtang/storage/samsung/lda_siamese/val/feat_ali298/iter500/pool6/1.npz'):
  f=open('../train/train_list.txt','r')
  list_197 = []
  for line in f.readlines():
    if 'query' in line:
      list_197+=os.listdir(line.split()[0])
  ...
  idx_list=[]
  for im in list_197:
    idx=list(img).index(str(im)[:-4])
    idx_list.append(idx)
  feat = feat[idx_list,:]
  label_array = label_array[idx_list]
  print feat.shape
...
to_path='/home/qtang/storage/samsung/lda_siamese/val/feat_evaLib/iter500/pool6_lda_197'
if not os.path.exists(to_path):
  os.makedirs(to_path)
for npz in os.listdir(os.path.join(from_path)):
  tp=np.load(os.path.join(from_path,npz))
