...
import copy
def str2attr(lin2):
    tmp_p=lin2.strip().split(',')
    attr=[]
    for p in tmp_p:
      attr.append(int(p))
    return attr

def txt2pkl(att_dict):
    lines=open(att_dict).readlines()
    new_dict={}
    for lin in lines:
      tmp_p=lin.split()
      new_dict.update({tmp_p[0]:str2attr(tmp_p[1])})
    return new_dict
...
class DataProvider:
    def __init__(self, batchsize=64, att_dict=None,list_path=None,mean_file_path=None,new_size=227):
        ...
        print 'att_dict'
        self.att_dict=txt2pkl(att_dict)
    ...
    def find_diff_list(self):
      ...
      for folder in all_img_dict:
        ...
        assert len_f >=2
        for d_i in range (10):
          tmp_folder=random.sample([_ for _ in folder_list  if _ != folder] ,1)[0]
    ...
    def create_batch(self,t_img_pair_list):
        ...
        sim     = (np.zeros(self.batchsize)).reshape(self.batchsize).astype(np.int64)
        ...
        for i in range(self.batchsize):
          if label1[i]==label2[i]:
            sim[i] = 1
        return img1,img2,label1,label2,attr1_0,attr1_1,attr1_2,attr1_3,attr1_4,attr1_5,attr1_6,attr1_7,attr2_0,attr2_1,attr2_2,attr2_3,attr2_4,attr2_5,attr2_6,attr2_7,sim
...
class DataProviderLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
      ...
      for i_ in range(5+self.att_num*2):
